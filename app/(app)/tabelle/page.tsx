...
const BKLASSE_TEAM_NAMES = ['SV Adelshofen II','SV Germering III','VSST Günzlhofen','[SG] TSV Herrsching/SF Breitbrunn 2','SV Kottgeisering','FC Landsberied II','SV Mammendorf','TSV Moorenweis II','SC Schöngeising II','TV Stockdorf II','TSV Türkenfeld II','SpVgg Wildenroth II']
      ...
      {!isB && playedMatchdays === 0 && ourPriorStandings.length > 0 ? (
        ...
        <>
          {topAttacks.length > 0 && topAttacks.some(t => t.gf > 0) && (
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700">
                <h2 className="font-bold text-gray-900 dark:text-gray-100">Beste Angriffe</h2>
              </div>
              <div className="divide-y divide-gray-50 dark:divide-gray-700">
                {topAttacks.map((s, i) => (
                  <div key={s.teamId} className="flex items-center px-4 py-2.5 gap-2">
                    ...
                    <div className="flex-1 text-sm font-medium text-gray-800 dark:text-gray-200 truncate">{s.teamName}</div>
                    <div className="text-sm font-bold text-gray-900 dark:text-gray-100">{s.gf}</div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            ...
            <div className="divide-y divide-gray-50 dark:divide-gray-700">
              {standings.map((s) => {
                ...
                return (
                  <div key={s.teamId} className={`px-4 py-3 ${isW ? 'bg-red-50 dark:bg-red-900/20' : ''}`}>
                    <div className={`flex items-center gap-2 font-semibold text-sm mb-2 ${isW ? 'text-red-700 dark:text-red-400' : 'text-gray-800 dark:text-gray-200'}`}>
                      <TeamLogo name={s.teamName} size="sm" className="flex-shrink-0" />
                      {s.teamName}
                    </div>
...
function StatCell({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="bg-gray-50 dark:bg-gray-700 rounded-lg py-1.5">
      <div className={`font-bold text-sm ${highlight ? 'text-red-700 dark:text-red-400' : 'text-gray-900 dark:text-gray-100'}`}>{value}</div>
      <div className="text-gray-400 dark:text-gray-500 text-[10px] leading-tight mt-0.5">{label}</div>
    </div>
  )
}
