...
export default async function TippsPage({
  ...
  if (isMatchdayComplete && matchdayMatchIds.length > 0) {
    ...
    if (recapBets && recapBets.length > 0) {
      ...
      const totalStaked = singleBets.reduce((s, b) => s + b.stake, 0) + recapCombos.reduce((s, c) => s + c.stake, 0)
      const totalLost = lostSingles.reduce((s, b) => s + b.stake, 0) + lostCombos.reduce((s, c) => s + c.stake, 0)
      ...
      if (spieltagskoenig || eierAusStahl || unluckyBastard || ergebnisOrakel || griffInsKlo || betonmischer || onFire) {
        recapData = { spieltagskoenig, eierAusStahl, unluckyBastard, ergebnisOrakel, griffInsKlo, betonmischer, onFire, stats }
      }
    }
  }
