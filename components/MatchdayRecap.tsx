...
function HighlightCard({
  ...
  return (
    <div className={`rounded-2xl border ${accentBorder} ${accentBg} px-4 py-3`}>
      <div className="text-xl mb-1.5">{emoji}</div>
      <div className="text-[10px] text-gray-500 font-semibold uppercase tracking-wide">{title}</div>
      <div className="font-bold text-gray-900 text-sm mt-0.5 truncate">{name}</div>
...
export function MatchdayRecap({ data, matchday }: { data: RecapData; matchday: number }) {
  const { mvp, bestOdds, unluckyBastard } = data
  if (!mvp && !bestOdds && !unluckyBastard) return null
      ...
      {unluckyBastard && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-4 py-3 bg-orange-50 border-b border-orange-100 flex items-center gap-2">
            ...
            <div>
              <div className="font-bold text-gray-900 text-sm">Unlucky Bastard</div>
          ...
          <div className="px-4 py-3 flex items-center justify-between">
            <div>
              <div className="font-semibold text-gray-900 text-sm">{unluckyBastard.name}</div>
